...
Launched Controllers:
    ...
    4. ** Trying to add forawrd command controller
...
def generate_launch_description():
    ...
    # NEW Load the forward position controller after launching the gripper controller
    load_forward_position_controller_cmd = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=start_gripper_action_controller_cmd,
            on_exit=[start_forward_position_controller_cmd]))

    # Create the launch description and populate
    ld = LaunchDescription()

    # Add the actions to the launch description in sequence
    # This loads the cintrollers.
    ld.add_action(delayed_start)
    ld.add_action(load_joint_state_broadcaster_cmd)
    ld.add_action(load_arm_controller_cmd)
    ld.add_action(load_forward_position_controller_cmd) # <-- Added to sequence
